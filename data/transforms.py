...
from .transfom_utils import box_xyxy_to_cxcywh
...
class MultiRandomResize:
    ...
    def __call__(self, imgs, infos):
        ...
        return zip(*[self._resize(img, info, size=new_size, max_size=self.max_size) for img, info in zip(imgs, infos)])

    def _resize(self, img, info, size, max_size):
        if isinstance(size, (list, tuple)):
            assert len(size) == 2, f"Size length should be 2, but get {len(size)}."
            new_hw = size[::-1]
        else:
            new_hw = self._get_new_hw(current_wh=img.size, new_short_size=size, new_long_max_size=max_size)
    ...
    @staticmethod
    def _get_new_hw(current_wh, new_short_size, new_long_max_size) -> [int, int]:
        w, h = current_wh
...
class MultiNormalize:
    def __init__(self, mean, std, save_unnorm=False):
        self.mean = mean
        self.std = std
        self.save_unnorm = save_unnorm
    ...
    def __call__(self, imgs: list, infos: list):
        ...
        def shift(image, info, region, target_h, target_w):
            ...
            if "boxes" in info:
                ...
                info["boxes"] *= torch.as_tensor(
                    [target_w / width, target_h / height, target_w / width, target_h / height])
...
def transforms_for_train(coco_size: bool = False, overflow_bbox: bool = False, reverse_clip: bool = False):
    ...
    return MultiCompose([
        ...
        MultiRandomSelect(
            MultiRandomResize(sizes=scales, max_size=1536),
            MultiCompose([
                MultiRandomResize([400, 500, 600] if coco_size else [800, 1000, 1200]),
                MultiRandomCrop(
        ...
        MultiCompose([
            MultiToTensor(),
            MultiNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        ]),
        MultiReverseClip(reverse=reverse_clip)
    ])


def transforms_for_eval():
    return MultiCompose([
        # MultiRandomResize(sizes=[864], max_size=1536),
        MultiRandomResize(sizes=[800], max_size=1536),
        MultiCompose([
            MultiToTensor(),
            MultiNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        ])
...
# def get_collate_fn(transforms: MultiCompose):
#     def collate_fn(batches):
#         batches_num = len(batches)
#         imgs = tuple(img for batch in batches for img in batch["imgs"])
#         infos = tuple(inf for batch in batches for inf in batch["infos"])
#         imgs, infos = transforms(imgs, infos)
#         for img, inf in zip(imgs, infos):
#             img.requires_grad_(False)
#         seq_len = len(imgs) // batches_num
#         collated_batch = {
#             "imgs": [imgs[i:i + seq_len] for i in range(0, len(imgs), seq_len)],
#             "infos": [infos[i:i + seq_len] for i in range(0, len(infos), seq_len)]
#         }
