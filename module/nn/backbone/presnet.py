...
class PResNet(nn.Module):
    def __init__(
        self,
        depth,
        variant='d',  # a:Standard ResNet
        num_stages=4,
        return_idx: list = [0, 1, 2, 3],
        act='relu',
        freeze_at=-1,
        freeze_norm=True,
        pretrained=False,
        ...
        if freeze_at >= 0:
            self._freeze_parameters(self.conv1)  # stage 0
        ...
        if pretrained:
            if isinstance(pretrained, bool) or 'http' in pretrained:
                state = torch.hub.load_state_dict_from_url(download_url[depth], map_location='cpu')
            else:
                if not os.path.isabs(pretrained):
                    pretrained = os.path.abspath(pretrained)
                if not os.path.exists(pretrained):
                    raise FileNotFoundError(f"Pretrained model file not found: {pretrained}")
    ...
    def forward(self, ntensor: NestedTensor):
        _x = ntensor.tensors
        conv1 = self.conv1(_x)
        _x = F.max_pool2d(conv1, kernel_size=3, stride=2, padding=1)
        outs = []
        for idx, stage in enumerate(self.res_layers):
            ...
            if idx in self.return_idx:
                outs.append(_x)
        res = []
        for name, output in zip(self.return_idx, outs):
            masks = ntensor.masks
            ...
            res.append(NestedTensor(output, masks))
        return res


def build(config):
    # load backbone
    sig = inspect.signature(PResNet)
    PResNet_cfg = {k: v for k, v in config['Backbone'].items() if k in sig.parameters}
    model = PResNet(**PResNet_cfg)
    return model
